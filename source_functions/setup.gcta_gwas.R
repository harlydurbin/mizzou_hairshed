...
drp %>%
  ...
  filter(!is.na(pheno)) %>%
  write_delim(here::here(glue("data/derived_data/gcta_gwas/{model}/pheno.txt")),
              col_names = FALSE)

drp %>%
  distinct(full_reg, iid) %>%
  write_delim(here::here(glue("data/derived_data/gcta_gwas/{model}/keep_list.txt")))
