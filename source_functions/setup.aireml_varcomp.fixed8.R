...
matched %>%
  ...
  write_csv(here::here("data/derived_data/aireml_varcomp/fixed8/sanity_key.csv"),
            na = "")

#'
## -----------------------------------------------------------------------------
matched %>%
  select(full_reg, year, calving_season, toxic_fescue, age_group, mean_apparent_high, mean_day_length, farm_id, hair_score) %>%
  write_delim(here::here("data/derived_data/aireml_varcomp/fixed8/data.txt"),
              col_names = FALSE)
