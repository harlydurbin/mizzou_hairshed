...
matched %>%
  ...
  write_csv(here::here("data/derived_data/base_varcomp/fixed5/sanity_key.csv"),
            na = "")

#'
## -----------------------------------------------------------------------------
matched %>%
  select(full_reg, year, calving_season, toxic_fescue, age_group, mean_day_length, hair_score) %>%
  write_delim(here::here("data/derived_data/base_varcomp/fixed5/data.txt"),
              col_names = FALSE)
