# snakemake -s source_functions/snp1101.snakefile -j 400 --rerun-incomplete --latency-wait 30 --config --cluster-config source_functions/cluster_config/snp1101.cluster.json --cluster "sbatch -p {cluster.p} -o {cluster.o} --account {cluster.account} -t {cluster.t} -c {cluster.c} --mem {cluster.mem} --account {cluster.account} --mail-user {cluster.mail-user} --mail-type {cluster.mail-type}" --until airemlf90 -p &> log/snakemake_log/snp1101/201014.snp1101.log
...
os.makedirs("log/psrecord/snp1101", exist_ok = True)
os.makedirs("log/psrecord/snp1101/airemlf90", exist_ok = True)
...
rule setup_snp1101:
	input:
		aireml_log = config['path'][wildcards.model],
	...
	shell:
		...
		Rscript --vanilla {input.script} {input.aireml_log}
		"""
