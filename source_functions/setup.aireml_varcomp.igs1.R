...
dat <-
  ...
  mutate(keep = case_when(cross %in% c("SIM", "RAN", "SH", "GEL", "CHIA") ~ TRUE,
                          source %in% c("American Simmental Association",
                                        ...
                                        "American Gelbvieh Association") ~ TRUE,
                          brd_source %in% c("American Simmental Association",
                                            "Red Angus Association of America",
                                            "American Shorthorn Association",
...
matched %>%
  ...
  write_csv(here::here("data/derived_data/aireml_varcomp/igs1/sanity_key.csv"),
            na = "")
...
matched %>%
  ...
  write_delim(here::here("data/derived_data/aireml_varcomp/igs1/data.txt"),
              col_names = FALSE)
