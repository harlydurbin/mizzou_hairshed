...
## -----------------------------------------------------------------------------------------------------------------------------------
dat <-
  readxl::read_excel(here::here("data/derived_data/ua_score_groups.xlsx")) %>% 
  bind_rows(readxl::read_excel(here::here("data/derived_data/score_groups.xlsx"))) %>%
  select(farm_id, date_score_recorded, score_group) %>% 
  mutate(date_score_recorded = lubridate::ymd(date_score_recorded)) %>% 
  right_join(cleaned) %>% 
  mutate(score_group = replace_na(1)) %>% 
  assertr::verify(!is.na(score_group))
...
matched %>% 
  ...
  write_csv(here::here("data/derived_data/base_varcomp/fescue1/sanity_key.csv"),
            na = "")
...
matched %>% 
  ...
  write_delim(here::here("data/derived_data/base_varcomp/fescue1/data.txt"),
              col_names = FALSE)
