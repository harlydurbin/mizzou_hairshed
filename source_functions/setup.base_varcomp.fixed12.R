...
matched %>%
  ...
  write_csv(here::here("data/derived_data/base_varcomp/fixed12/sanity_key.csv"),
            na = "")
...
matched %>%
  ...
  filter(n() == 1) %>%
  ungroup() %>%
  select(full_reg, cg_num, mean_apparent_high, mean_day_length, hair_score) %>%
  write_delim(here::here("data/derived_data/base_varcomp/fixed12/data.txt"),
              col_names = FALSE)
