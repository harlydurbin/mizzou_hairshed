...
matched %>% 
  ...
  write_csv(here::here("data/derived_data/random_regression/length1/sanity_key.csv"),
            na = "")
...
matched %>% 
  ...
  ungroup() %>% 
  mutate(intercept == 1) %>% 
  select(full_reg, year, calving_season, toxic_fescue, age_group, mean_day_length, intercept, hair_score)
  write_delim(here::here("data/derived_data/random_regression/length1/data.txt"),
              col_names = FALSE)
