...
sav_all <-
  ...
  purrr::map(function(.x) {
    ...
    (nms <-
       names(
         read_excel(
           "/Users/harlyjanedurbin/Box Sync/HairShedding/ReportedData/UniversityofArkansas/Savoy Data 1114016 lrm.xlsx",
           sheet = .x
         )
       ))
...
bat_16 <-
  ...
  purrr::map(function(.x) {
    ...
    (nms <-
       names(
         read_excel(
           "/Users/harlyjanedurbin/Box Sync/HairShedding/ReportedData/UniversityofArkansas/LFST all genotyped lrm 11142016_JEK.xlsx",
           sheet = .x
         )
       ))
    # Assign column types based on column name
    ...
    read_excel(
      ...
      dplyr::mutate(
                    source_file = "LFST all genotyped lrm 11142016_JEK.xlsx",
                    source_tab = .x)
  }) %>%
  purrr::map(function(df = .x) {
    dates <-
...
c("/Users/harlyjanedurbin/Box Sync/HairShedding/ReportedData/UniversityofArkansas/GeneSeek plate sheet final computer version 10052016 (2).xlsx") %>% 
  ...
  select(-animal_id) %>% 
  rename(
    # U of A ID original --> animal ID
    animal_id = 4,
    # ID for Neogen --> registration_number
  ...
  mutate(date_score_recorded = lubridate::ymd(date_score_recorded),
         ...
         year = 
           case_when(
             str_detect(notes, "20[[:digit:]]{2}") ~ as.integer(str_extract(notes, "20[[:digit:]]{2}")),
...
bat_17 <-
  excel_sheets(
  "/Users/harlyjanedurbin/Box Sync/HairShedding/ReportedData/UniversityofArkansas/UA_Batesville_2017HS_JEK.xlsx"
) %>%
  purrr::set_names() %>%
  purrr::map(
    ~ read_excel(
  ...
  purrr::map(# Pivot hair score columns from wide to long
  ...
  purrr::imap(
    ~ select(
      ...
      mutate(
        date_score_recorded = janitor::excel_numeric_to_date(as.numeric(date_score_recorded)),
        hair_score = as.integer(hair_score),
  ...
  mutate(farm_id = "BAT",
         ...
         calving_season = 
           case_when(
             str_detect(comment, "Sp|sp") ~ "SPRING",
             str_detect(comment, "fall|Fall") ~ "FALL"),
         age = 
           case_when(
             comment == "first calf heifers" ~ "2",
...
sav_18 <-
  ...
  mutate(
    date_score_recorded = 
      case_when(
        name == "March" ~ lubridate::ymd("2018-03-15"),
  ...
  mutate(
    animal_id =
      case_when(
        ...
        is.na(animal_id) & !is.na(registration_number) ~ str_extract(registration_number, "(?<=SAV|BAT)[[:alnum:]]+$"),
        TRUE ~ animal_id
      ),
    sex = 
      case_when(
        # If sex missing but not reg, pull sex from reg
        is.na(sex) & !is.na(registration_number) ~ str_extract(registration_number, "F|M"),
        TRUE ~ sex
        ),
    # Correctly formatted animal ID
    f_animal_id = 
      case_when(
        !is.na(animal_id) ~ str_pad(animal_id, width = 4, side = "left", pad = "0")),
    # Correctly formatted reg
    f_registration_number =
      case_when(
        !is.na(registration_number) ~ glue::glue("{str_extract(registration_number,'F|M')}00UA-{farm_id}{f_animal_id}"),
...
# Examine raw combined dataframe
...
# Match to lab IDs, further cleaning

```{r}
ua_clean <-
  ua_join %>%
  # distinct(farm_id, animal_id, f_animal_id, registration_number, f_registration_number, dna_id) %>%
  # f_reg --> Reg
  id_search(
    source_col = f_registration_number,
...
# If more than one breed code, call it CROS

```{r}
ua_clean %<>% 
  mutate(breed_code = 
           case_when(
             breed_code %in% c("Hereford", "HFD") ~ "HFD",
