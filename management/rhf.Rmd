...
rhf_breed <-
  ...
  bind_rows(read_excel("~/Box Sync/HairShedding/ReportedData/2018/DataRecording_RHF_2018.xlsx",
              ...
              left_join(cleaned %>% 
                          ...
                          filter(farm_id == "RHF"))) %>% 
  filter(!is.na(breeds)) %>% 
  mutate(cross = case_when(breeds %in% c("100% AR", "PB AR") ~ "RAN",
                           breeds %in% c("PB AN") ~ "AN",
                           breeds %in% c("PB SM") ~ "SIM",
                           TRUE ~ "cross")) %>% 
  select(farm_id, breed_code, registration_number, animal_id, Lab_ID, temp_id, cross)

```

```{r}
write_rds(rhf_breed, here::here("data/raw_data/201005.rhf_breed.rds"))
```
