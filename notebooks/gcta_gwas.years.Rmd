...
I got 29 SNPs back from --cojo-slct. Here’s where the selected SNPs on chromosome 5 and 23 fall, projected onto my GWAS results from the SNP1101 GWAS. Working on looking at the COJO p-values for non-selected SNPs (only 743,290 so about 4,000 fewer than the input - not sure why).
...
years_metasoft <- 
  ...
  mutate(neglog10q_re = -log10(qvalue::qvalue(pvalue_re2)$qvalues))

```

## Calculate allele frequencies and missingness in `METASOFT animals` for use in COJO
...
```
srun --account animalsci -p BioCompute,htc4,hpc5 -t 12:00:00 --mem 48G -c 56 source_functions/gcta_1.93.2beta/gcta64 --bfile data/raw_data/geno_dump/200924_HairShed.850K.qc --cojo-file data/derived_data/cojo/years/years.ma --cojo-slct --out data/derived_data/cojo/years/years --threads 56
```

* Run COJO again using only year-by-year GWAS animals as LD ref
...
fixed9_snp1101 %>% 
  ...
  hair_manhattan(y_var = neglog10q,
                 ...
                 highlight = TRUE,
                 highlight_col = clr,
                 plot_title = "LD calculated in full dataset") 

ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_slct.png"),
       width = 8,
       height = 3)
```
...
fixed9_snp1101 %>% 
  ...
  hair_manhattan(y_var = neglog10q,
                 ...
                 highlight = TRUE,
                 highlight_col = clr,
                 plot_title = "LD calculated in subset") 

ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_slct.subset.png"),
       width = 8,
       height = 3)
```
...
fixed9_snp1101 %>% 
  ...
  labs(x = NULL,
       ...
       title = "LD calculated in full dataset") +
...
```{r}
ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_chr5.png"),
       width = 8,
       height = 3)
```
...
fixed9_snp1101 %>% 
  ...
  geom_point(alpha = 0.75) +
  scale_color_identity() +
  geom_point(data = years_subset_jma %>% 
               filter(chr == 5) %>% 
               select(chr, pos) %>% 
               left_join(fixed9_snp1101 %>% 
                           select(chr, pos, neglog10q)),
...
```{r}
ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_chr5.subset.png"),
       width = 8,
       height = 3)
```
...
```{r}
ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_chr23.png"),
       width = 8,
       height = 3)
```
...
```{r}
ggsave(filename = here::here("figures/cojo/fixed9_snp1101.q.cojo_chr23.subset.png"),
       width = 8,
       height = 3)
```
