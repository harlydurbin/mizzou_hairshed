...
# robust joint test using 2018 day length

1. subset to top 100 significant SNPs, animals with phenotypes

```{r}
fixed9_snp1101 %>% 
  top_n(100, neglog10q) %>% 
  arrange(chr, pos) %>% 
  mutate(end = pos, set = row_number()) %>% 
  select(chr, pos, end, set) %>% 
  write_delim(here::here("data/derived_data/robust_joint_gxe/keep_range.txt"),
              delim = " ", 
              col_names = FALSE)
```

```{r}
read_table2(here::here("data/derived_data/robust_joint_gxe/gxe_gwas.day_length.2018.fam"),
            col_names = FALSE) %>% 
  filter(!is.na(X6)) %>% 
  select(X1, X2) %>% 
  write_delim(here::here("data/derived_data/robust_joint_gxe/keep.txt"),
              delim = " ", 
              col_names = FALSE)
```


plink --bfile data/derived_data/robust_joint_gxe/gxe_gwas.day_length.2018 --cow --double-id --keep data/derived_data/robust_joint_gxe/keep.txt --make-bed --out data/derived_data/robust_joint_gxe/test

```{r}
read_table2(here::here("data/derived_data/robust_joint_gxe/test.fam"),
            col_names = FALSE) %>% 
  select(FID = X1, IID = X2) %>% 
  left_join(read_table2(here::here("data/derived_data/robust_joint_gxe/gxe_gwas.day_length.2018.fam"),
                        col_names = FALSE) %>% 
              select(FID = X1, IID = X2) %>% 
              bind_cols(read_table2(here::here("data/derived_data/robust_joint_gxe/gxe.txt"),
                                    col_names = "C1"))) %>% 
  assertr::verify(!is.na(C1)) %>% 
  write_delim(here::here("data/derived_data/robust_joint_gxe/test.cov"),
              col_names = TRUE,
              delim = " ")
```
...
R --vanilla < source_functions/load_rserve.R
plink --bfile data/derived_data/robust_joint_gxe/test --allow-no-sex --cow --double-id --covar data/derived_data/robust_joint_gxe/test.cov --R source_functions/robust-joint-int-plugin.R

```{r}
gxe_robust <-
  read_table2(here::here("plink.auto.txt"),
            col_names = c("chr", "snp_id", "pos", "minor_allele", "p_model", "p_robust")) %>% 
  mutate(neglog10q = -log10(qvalue::qvalue(p_robust)$qvalues)) %>% 
  arrange(desc(neglog10q))
```

```{r}
ggqq(gxe_robust$p_model)
```
