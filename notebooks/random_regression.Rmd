...
snp_sol <-
  read_table2(here::here("data/derived_data/snp1101/length1/out/gwas_ssr_length1_bvs_p.txt"))  %>%
  janitor::clean_names() %>%
  mutate(neglog10p = -log10(p_value),
         abs_snp = abs(b_value)) %>%
  left_join(read_table2(here::here("data/derived_data/snp1101/length1/out/gwas_ssr_length1_bvs.txt"),
                        skip = 12) %>%
...
# GWAS using `length1` reaction norm slopes
