...
* What's the correlation between their p-values?

```{r}
years_metasoft %>% 
  select(chr, pos, meta = neglog10q) %>% 
  left_join(fixed9_snp1101 %>% 
              select(chr, pos, neglog10q)) %>% 
  summarise(`Corr. between p-values` = cor(neglog10q, meta))
```
...
All results below projected on Metasoft results, not results from SNP1101 using DEBVs
...
```{r, fig.width=8, fig.height=9, eval = TRUE}
...
ggsave(filename = here::here("figures/cojo/metasoft.q.cojo_panel.png"), height = 9, width = 8) 
