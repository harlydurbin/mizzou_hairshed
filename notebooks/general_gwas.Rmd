...
## Correlation, adjusted and un-adjusted p-values

```{r}
fixed9_snp1101 %>% 
  select(chr, pos, adj_p) %>% 
  left_join(fixed9_no_pa %>% 
              select(chr, pos, p_value)) %>% 
  summarise(cor(adj_p, p_value))
```
...
ggsave(here::here("figures/general_gwas/fixed9_snp1101.with_pa.adj_p.Q-Q.png"))
```
