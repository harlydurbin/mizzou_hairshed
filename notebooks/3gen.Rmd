...
bg_ped <-
  ...
  janitor::remove_empty(which = "rows") %>% 
  group_by(registration_number) %>% 
  fill(c("Lab_ID", "farm_id", "animal_id", "temp_id", "dob", "sex", "sire_reg", "dam_reg"), .direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
  # Lab_ID search
  id_search(
...
an_ped <-
  ...
  group_by(registration_number) %>% 
  fill(c("Lab_ID", "farm_id", "temp_id", "animal_id", "sire_reg", "dam_reg"), .direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
...
full_ped %<>%
  # Prioritize Lab IDs
  mutate(
    Lab_ID_final = case_when(
      # If original Lab ID missing sample
      is.na(assay_orig) & !is.na(assay_dup) ~ duplicate,
      # If duplcate sample has F250 or HD
      str_detect(assay_dup, "F250|HD") &
        call_rate_dup > 0.89 ~ duplicate,
      TRUE ~ Lab_ID
    )
  ) %>% 
  group_by(full_reg) %>% 
