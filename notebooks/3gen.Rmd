...
sh_ped %<>% 
  mutate(sex = case_when(
    registration_number %in% sh_ped$sire_reg ~ "M",
...
gel_ped <-
  ...
  # Add birth date info
  left_join(read_excel(here::here("data/raw_data/200413.GEL.DurbinHarly_HairShedPed_04132020.xls")) %>%
...
cha_ped <-
  # Import raw data
  read_table2(here::here("data/raw_data/200416.CHA.AICA_MUhairshed_pedigree_04162020.txt"),
              skip = 1,
...
hfd_ped <-
  # Import raw pedigree
  read_csv(here::here("data/raw_data/200415.HFD.MU Shedding project pedigree 041520.CSV"),
...
sim_ped <-
  # Import raw pedigree
  read_csv(here::here("data/raw_data/200409.SIM.three-gen-ped.csv"),
           col_types = cols(.default = col_character()),
           skip = 1,
...
ran_ped <-
  ...
  left_join(read_excel(here::here("data/raw_data/200413.RAN.Hair Shedding Pedigree 4.13.20.xlsx"),
                       col_types = "text") %>%
...
full_ped %<>%
  ...
  ungroup() %>% 
  distinct() %>% 
  mutate(dam_reg = if_else(dam_reg == registration_number,
                           "0",
                           dam_reg)) %>% 
...
```{r}
full_ped %<>% 
  left_join(read_csv(here::here("data/raw_data/201027.SIM.6-gen-ped.csv")) %>% 
  ...
  mutate(sex = case_when(is.na(sex) & !is.na(animal) ~ str_extract(animal, "(?<=USA)M|F"),
                     TRUE ~ sex),
     sire_reg = case_when(!is.na(sire) & !str_detect(sire, "CAN") ~ 
                            glue("{str_extract(sire, '^[[:upper:]]{3}')}{str_extract(sire, '(?<=0)[1-9][[:digit:]]+$')}"),
                          TRUE ~ sire_reg),
     sire_reg = str_replace(sire_reg, "^AAN", "AAA"),
     dam_reg = case_when(!is.na(dam) & !str_detect(dam, "CAN") ~ 
...
```{r}
full_ped %<>% 
  left_join(read_csv(here::here("data/raw_data/201027.RAN.All_Animals_SireDam.csv")) %>%
              janitor::remove_empty(which = c("rows", "cols")) %>% 
              mutate(full_reg = glue("RAN{anm_key}")) %>% 
              select(full_reg, sire_key, dam_key)) %>% 
  mutate(sire_reg = case_when(!is.na(sire_key) & !str_detect(sire_reg, "^AAA") ~ glue("RAN{sire_key}"),
                              TRUE ~ sire_reg),
         dam_reg = case_when(!is.na(dam_key) & !str_detect(dam_reg, "^AAA") ~ glue("RAN{dam_key}"),
                              TRUE ~ dam_reg)) %>% 
...
```{r}
full_ped %<>% 
  left_join(readxl::read_excel(here::here("data/raw_data/201027.ped_fixes.xlsx"))) %>% 
  mutate(sire_reg = if_else(!is.na(misc_sire), misc_sire, sire_reg),
         dadm_reg = if_else(!is.na(misc_dam), misc_dam, dam_reg)) %>% 
