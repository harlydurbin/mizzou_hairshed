---
title: "Multi-breed 3-generation pedigree for 2020 update email"
author: "Harly Durbin"
...
sh_ped <-
  purrr::pmap(
    ...
    .f = function(x, y, z) {
      read_excel(here::here("data/raw_data/200417.SH.Mizzou41620.xlsx")) %>%
        select(
...
gel_ped <-
  purrr::pmap(
    ...
    .f = function(x, y, z) {
      read_excel(here::here(
        "data/raw_data/200413.GEL.DurbinHarly_HairShedPed_04132020.xls"
      )) %>%
        select(
          registration_number = x,
          sire_reg = y,
  ...
  left_join(
    cleaned %>%
      left_join(animal_table %>%
                  select(Lab_ID, breed_assoc_reg, breed_assoc)) %>%
      filter(breed_code == "GEL" |
               str_detect(breed_assoc, "Gelbvieh")) %>%
      filter(!farm_id %in% c("SAV", "BAT")) %>%
      filter(
        is.na(breed_assoc) |
          str_detect(breed_assoc, "American Gelbvieh Association")
      ) %>%
      mutate(
        registration_number = if_else(
          !str_detect(registration_number, "^AMGV"),
...
cha_ped <-
  read_table2(
    here::here(
      "data/raw_data/200416.CHA.AICA_MUhairshed_pedigree_04162020.txt"
    ),
    skip = 1,
  ...
  left_join(
    cleaned %>%
      ...
      filter(
        breed_code == "CHA" |
          breed_assoc == "American International Charolais Association"
      )  %>%
      filter(!farm_id %in% c("BAT", "SAV", "HDF")) %>%
      filter(!is.na(registration_number)) %>%
      filter(!str_detect(registration_number, "^6")) %>%
...
cleaned %>%
  ...
  filter(!is.na(registration_number)) %>% 
  select(registration_number) %>% 
  write_csv(here::here("data/derived_data/3gen/200413.HFD_reg.csv"), col_names = FALSE)
```
...
hfd_ped <-
  read_csv(here::here(
  "data/raw_data/200415.HFD.MU Shedding project pedigree 041520.CSV"
  ...
  left_join(
    cleaned %>%
      left_join(animal_table %>%
                  select(Lab_ID, breed_assoc)) %>%
      filter(breed_code == "HFD" |
               str_detect(breed_assoc, "Hereford"))  %>%
      filter(!farm_id %in% c("BAT", "SAV")) %>%
      distinct(Lab_ID, registration_number, farm_id, animal_id, temp_id) %>%
      filter(!is.na(registration_number))
  ) %>% 
  mutate_at(vars(contains("reg")), ~ if_else(is.na(.), "0", as.character(glue("HER{.}"))))
```
  ...
  function(file_path) {
    ...
    bg_var <-
      tribble(
        ...
        # animal
        "Sire Reg No",
        "Paternal Grandsire Reg No",
        "Paternal Granddam Reg No",
        # sire
        "Dam Reg No",
        "Maternal Grandsire Reg No",
        "Maternal Granddam Reg No",
        # dam
        "Paternal Grandsire Reg No",
        "Paternal Great Paternal Grandsire Reg No",
        "Paternal Great Paternal Granddam Reg No",
        # pgs
        "Paternal Granddam Reg No",
        "Paternal Great Maternal Grandsire Reg No",
        "Paternal Great Maternal Granddam Reg No",
        # pgd
        "Maternal Grandsire Reg No",
        "Maternal Great Paternal Grandsire Reg No",
        "Maternal Great Paternal Granddam Reg No",
        # mgs
        "Maternal Granddam Reg No",
        "Maternal Great Maternal Grandsire Reg No",
        "Maternal Great Maternal Granddam Reg No",
        # mgd
      )
    ...
    long <-
      ...
      mutate_at(vars(contains("reg")), ~ str_replace(., "^EA", "AAA")) %>%
      left_join(cleaned %>%
...
```{r, message=FALSE, warning=FALSE}
bg_ped <-
  list.files("~/Box Sync/HairShedding/ReportedData/IBBA", full.names = TRUE, pattern = "details") %>% 
  purrr::map(~ gather_brangus(file_path = .x)) %>% 
  reduce(bind_rows) %>% 
  janitor::remove_empty(which = "rows") %>% 
  group_by(registration_number) %>% 
  fill(c("Lab_ID", "farm_id", "animal_id", "temp_id", "dob", "sex"), .direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate_at(vars(contains("reg")), 
            ~ case_when(
...
```{r, message=FALSE, warning=FALSE}
sim_ped <-
  read_csv(here::here("data/raw_data/200409.SIM.three-gen-ped.csv"), col_types = cols(.default = col_character()), skip = 1, col_names = c("registration_number", "sire_reg", "dam_reg")) %>% 
  left_join(cleaned %>% 
...
ran_ped <-
  purrr::pmap(
    ...
    .f = function(x, y, z) {
      read_excel(
        here::here(
          "data/raw_data/200413.RAN.Hair Shedding Pedigree 4.13.20.xlsx"
        ),
        col_types = "text"
      ) %>%
        select(
  ...
  reduce(bind_rows) %>% 
  filter(!is.na(registration_number)) %>%
  # only send dam dob for some reason
  left_join(
    read_excel(
      here::here(
        "data/raw_data/200413.RAN.Hair Shedding Pedigree 4.13.20.xlsx"
  ...
  left_join(
    cleaned %>%
      ...
      filter(breed_code == "ANR" |
               str_detect(breed_assoc_reg, "Red Angus")) %>%
      filter(
        is.na(breed_assoc) |
  ...
  group_by(registration_number) %>%
  fill(c("Lab_ID", "farm_id", "temp_id", "animal_id", "dob" ,"sex"), .direction = "downup") %>%
  ungroup() %>%
  distinct() %>%
  mutate(dob = lubridate::mdy(dob),
         sex = case_when(
  ...
  mutate_at(vars(contains("reg")), ~ if_else(is.na(.), "0", as.character(glue("RAN{.}"))))
```
...
```{r, eval=FALSE}
an_reg %>% 
   write_csv(here::here("data/derived_data/3gen/200413.AN_reg.csv"), col_names = FALSE)
```
...
an_ped <-
  ...
  group_by(registration_number) %>% 
  fill(c("Lab_ID", "farm_id", "temp_id", "animal_id"), .direction = "downup") %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(source = "American Angus Association")
```
...
full_ped <-
  ...
  mutate(sire_reg = if_else(!is.na(an_reg), an_reg, sire_reg)) %>% 
  select(-an_reg) %>% 
  left_join(
    an_reg %>%
      filter(!is.na(an_reg)) %>%
  ...
  mutate(dam_reg = if_else(!is.na(an_reg), an_reg, dam_reg),
         source = "American Simmental Association") %>% 
  select(-an_reg) %>% 
  mutate_at(vars(contains("reg")), ~ case_when(
    is.na(.) ~ "0",
...
# Export for Helen and Bob

```{r}
full_ped %>% 
  mutate_at(vars(contains("reg")), ~ str_replace(., "^AAA", "AAN")) %>% 
  group_by(registration_number) %>% 
  arrange(source) %>% 
  slice(1) %>% 
  ungroup() %>% 
  select(Lab_ID, farm_id, animal_id, dob, sex, registration_number, sire_reg, dam_reg, source) %>% 
  arrange(Lab_ID, registration_number) %>% 
  id_search(source_col = registration_number,
            search_df = animal_table %>% 
              mutate(breg2 = as.character(glue("{international_breed_code}{breed_assoc_reg}"))),
            search_col = breg2,
            key_col = Lab_ID) %>% 
  group_by(registration_number) %>% 
  arrange(Lab_ID) %>% 
  slice(1) %>% 
  ungroup() %>% 
  writexl::write_xlsx(here::here("data/derived_data/3gen/200421.full_ped.xlsx"))
...
orphaned <-
  ...
  anti_join(full_ped %>% 
              distinct(Lab_ID, farm_id, animal_id, temp_id)) %>% 
  left_join(animal_table %>% 
              select_at(vars(BC, breed_assoc, breed_assoc_reg, international_breed_code, contains("sire"), contains("dam"), Lab_ID))) %>% 
  mutate(sire_reg = 
           case_when(
             !is.na(sire_breed_assoc_reg) ~ sire_breed_assoc_reg, 
             !is.na(Sire_Reg) ~ Sire_Reg,
             ),
         dam_reg = 
           case_when(
             !is.na(dam_breed_assoc_reg) ~ dam_breed_assoc_reg,
...
```{r}
  ...
  mutate_at(vars(contains("reg")), ~ str_remove_all(., "[[:punct:]]|x|X")) %>%
  # Remove "P" from Hereford
  mutate_at(vars(contains("reg")), ~ if_else(BC == "HFD", str_remove_all(., "P|p"), .)) %>%
  # Breed association codes
  mutate(
    sire_assoc = if_else(!is.na(sire_international_breed_code), sire_international_breed_code, NA_character_),
    dam_assoc = if_else(!is.na(dam_international_breed_code), dam_international_breed_code, NA_character_),
    breed_assoc = if_else(!is.na(international_breed_code), international_breed_code, NA_character_)) %>% 
  mutate_at(vars(contains("assoc")), ~ str_replace(., "^AAN", "AAA")) %>%
  # Imputing
  mutate(
    registration_number =
      case_when(
        !is.na(breed_assoc) ~ glue("{breed_assoc}{registration_number}"),
    ...
    dam_reg = 
      case_when(
        !is.na(dam_assoc) ~ glue("{dam_assoc}{dam_reg}"),
    ...
    sire_reg = 
      case_when(
        !is.na(sire_assoc) ~ glue("{sire_assoc}{sire_reg}"),
  ...
  mutate_at(vars(contains("reg")), ~ as.character(.)) %>% 
  # Filtering
  mutate(
    registration_number = case_when(
      is.na(farm_id) & !str_detect(registration_number, "^BIR|^SIM|^HER|^RAN|^BSH|^CIA|^RDP|^CHA|^AAA|^BGR") ~ NA_character_,
      is.na(registration_number) & !is.na(farm_id) ~ as.character(glue("{farm_id}{animal_id}{temp_id}")),
      TRUE ~ registration_number),
    sire_reg = if_else(str_detect(sire_reg, "^BIR|^SIM|^HER|^RAN|^BSH|^CIA|^RDP|^CHA|^AAA|^BGR"), sire_reg, NA_character_),
    dam_reg = if_else(str_detect(dam_reg, "^BIR|^SIM|^HER|^RAN|^BSH|^CIA|^RDP|^CHA|^AAA|^BGR"), dam_reg, NA_character_)) %>% 
  filter(!is.na(registration_number)) %>% 
...
full_ped %<>%
  ...
  fill(farm_id, animal_id, temp_id, .direction = "downup") %>% 
  mutate(
    sire_reg = case_when(
      n_distinct(sire_reg) > 1 ~ "0",
      TRUE ~ sire_reg
      ),
    dam_reg = case_when(
      n_distinct(dam_reg) > 1 ~ "0",
      TRUE ~ dam_reg
      )) %>% 
  ungroup() %>% 
  distinct() %>% 
  ...
  mutate(full_reg = case_when(
    n_distinct(full_reg) > 1 & !is.na(animal_id) & str_detect(full_reg, "SIM") ~ "0",
    TRUE ~ full_reg
  )) %>% 
  ungroup() %>% 
  filter(full_reg != "0")
...
```{r}
full_ped %>% 
  filter_at(vars(contains("reg")), any_vars(str_detect(., "NA")))
