...
lrt_calc(ll_null = 94375.966, ll_test = 93532.558)
...
lrt_calc(ll_null = 94966.416, ll_test = 93532.558)
...
## Most complicated model: temperature + day length + temperature*day length (`fixed4`)

```{r, warning=FALSE, message=FALSE}
read_table2(here::here("data/derived_data/base_varcomp/fixed4/solutions"),
            skip = 1,
            col_names = c("trait", "effect", "id_renamed", "solution", "se")) %>%
...
lrt_calc(ll_null = 93532.558, ll_test = 93506.058)
...
```{r}
tibble(Model = c("fixed4", "fixed3")) %>%
  mutate(AIC = purrr::map_dbl(.x = Model,
                              ~ parse_loglik(path = here::here(glue("data/derived_data/base_varcomp/{.x}/airemlf90.{.x}.log")),
                                             option = "AIC")),
         Model = case_when(Model == "fixed3" ~ "Temperature + day length",
                           Model == "fixed4" ~ "Temperature + day length + temperature*day length")) 
```
...
lrt_calc(ll_null = 104405.96, ll_test = 92222.897)
...
lrt_calc(ll_null = 93532.558, ll_test = 92222.897)
...
lrt_calc(ll_null = 82482.404, ll_test = 81482.508)
...
lrt_calc(ll_null = 82482.404, ll_test = 81474.369)
...
lrt_calc(ll_null = 81482.508, ll_test = 81476.451)
...
lrt_calc(ll_null = 81474.369, ll_test = 81476.451)
